...
- **Speed Index en el preview de Vercel (~3100-3180 ms) es más alto que en dev-server (~1640 ms) y no es CDN fría.** Repetido con la CDN ya caliente: mediana 3126 ms, misma zona que la primera tanda (3179 ms). Es real, no artefacto de primera visita. Sin diagnosticar — no se ha tocado.
- **El CLS de 0,0019 del preview no es la apertura.** Aparece igual con `prefers-reduced-motion`, así que no puede ser un elemento del overlay. `layout-shifts` de Lighthouse lo señala en `body > main#main > section#s2 > div.s2__listado` (el listado de obras). Sin arreglar — solo identificado.
